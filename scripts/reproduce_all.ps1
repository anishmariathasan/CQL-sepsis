...
if (-not $SkipData) {
    Write-Host ""
    Write-Host "Step 2: Collecting Offline Data (using EXPERT policy)" -ForegroundColor Cyan
    Write-Host "----------------------------------------"
    
    # Use the real clinician expert policy for true offline RL
    python scripts/02_collect_offline_data.py --policy_type expert --n_episodes $DATA_EPISODES --save_path data/offline_datasets/behavior_policy.pkl --verbose
    if ($LASTEXITCODE -ne 0) {
